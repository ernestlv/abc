...
so.rest = {
        ...
        requestModifyAssets: function( field, handler ){
            var value = CQ.Ext.getCmp( field.id ).getValue();
            if (!value){
                alert('You need to chose a field and value to modify.');
                return;
            }
        ...
        requestHistory: function( uri, handler ){
                ...
                // test kickoff  
                var data= JSON.stringify(requestObj); //make into data package
...
/////////////// REST HANDLERS FOR FIELD SERVICE /////////////////////////
        ...
        getHistory: function( evt ){
                ...
                var row = $CQ( evt.target ).attr( 'data-sni-row' ) - 1;
...
//every filter is correctly evaluated every time. The order filters are evaluated are based on your visual index inclusions first exclusions second.
        ...
        handleModifyAssets:function(data){
                  var result = so.g.getResult(); //pointing to iframe if no iframe pointing to special obj with contentWindow prop
                  var dashboard = so.g.getDashboard(); //if we are using iFrames dashboard will not be null then it shows the loading icon running in the dashboard
                  if ( dashboard ){
                    dashboard.style.display = 'block';
                    result.style.display = 'none';
                  }
                  result.contentWindow.location.reload( true );
