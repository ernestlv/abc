result = {
        ...
        getMaxSplit: function (grid, id){
            ...
            var p = result.grid.panel, z = p.length, i;
            for(i=0; i<z; i++){
                if ( p[i].contentID === id ){
                    return p[i].size;
                }
        ...
        clickSplit: function ( e ){
                        ...
                        var max = result.getMaxSplit(result.grid, id);
                        if (c.offsetWidth === 0){
                            c.style.width = max;
                            i.innerHTML = '-';
                        }else{
                            result.setMaxSplit(result.grid, id, c.style.width);
                            c.style.width = 0;
                            i.innerHTML = '+';
        ...
        doRows: function ( p, tbl, w ){
            ...
            result.addClass(tbl, h);
            result.addStyle(tbl, h);
            ...
            CQ.Ext.each(tbl.columns, function(c, i, a){
                ...
                result.addClass(c, div);
                result.addStyle(c, div);
        ...
        doPanel: function (p, w){
                        ...
                        if (p.split !== false){
                            var s = result.doSplit(p, w);
                        }
                        var c = result.doContent(p, w);
        },
		...
		loadGrid: function (){
                    ...
                    var store= new CQ.Ext.data.ArrayStore({
                        fields: result.grid.fields
                    });

		            // manually load local data
		            store.loadData(result.get());

		            var gcol = 0; //global col counter
		            CQ.Ext.each(result.grid.panel, function(p, i, a){
    ...
    adjustAttributePanel: function(l, s, w){
            ...
            $CQ(l).click( result.clickSplitSearch );
